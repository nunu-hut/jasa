...
if (isset($_POST['tambah'])){
	...
	$gabung_foto = $newfoto1 . "-" . $newfoto2 . "-" . $newfoto3 . "-" . $newfoto4;
	$edit = mysqli_query($mysqli, "UPDATE jasa SET foto='$gabung_foto',nama_usaha='$nama_usaha',nama_jasa='$nama_jasa',harga='$harga',jadwal='$jadwal',informasi='$informasi',keterangan='$keterangan',latitude='$latitude',longitude='$longitude' WHERE id_jasa = '$id' ") or die(mysqli_error($mysqli));;
	...
	header("location: index?id=".urlencode($id));
/*Kodingan edit Berakhir di sini */

} else if (isset($_GET['id'])) {
	$id = $_GET['id'];

	$hapus = mysqli_query($mysqli, "DELETE from jasa WHERE id_jasa = '$id'");
	
	header("location: index?pesan=delete");
} elseif (isset($_GET['status'])) {
	$id = $_GET['status'];
	mysqli_query($mysqli, "UPDATE jasa SET status=1 WHERE id_jasa='$id'");
